...
* `Validation record`: the DNS record that is used to prove ownership of a domain name ({{RFC9499}}). It typically contains an unguessable value generated by the provider which serves as a challenge. The provider looks for the validation record in the zone of the domain name being verified and checks if it contains the unguessable value.

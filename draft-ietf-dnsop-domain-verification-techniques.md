...
A unique token should be used in the challenge. It should be a random value issued between the Application Service Provider and the User with the following properties:
...
Application Service Providers should use authenticated channels to convey instructions and random tokens to Users. Otherwise, an attacker in the middle could alter the instructions, potentially allowing the attacker to provision the service instead of the User.
